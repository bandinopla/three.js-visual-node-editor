    ...
    implements IOverlayRenderer
{
    ...
    constructor(
        private onColorChange?: (color: Color, opacity: number) => void,
        transparent = true,
    ) {
        super();
        ...
        this.alphaSlider = new DraggableValue(
            'Alpha',
            true,
            0,
            1,
            0.01,
            this.onAlphaSlider.bind(this),
        );
        this.alphaSlider.enabled = transparent;

        this.RGBSliders = ['R', 'G', 'B'].map(
            (name, i) =>
                new DraggableValue(
                    name,
                    true,
                    ...
                    1,
                    0.01,
                    this.onRGBSlider.bind(this, i),
                ),
        );
        this.HSLSliders = ['H', 'S', 'L'].map(
            (name, i) =>
                new DraggableValue(
                    name,
                    true,
        ...
        this.overlay = new Column([
            new Row(
                [
                    new Column([this.colorSwatch, this.colorWheel], {
                        align: 'stretch',
                    }),
    ...
    renderOverlay(ctx: CanvasRenderingContext2D): void {
        ...
        this.overlay.render(
            ctx,
            this.overlay.width(ctx),
            this.overlay.height(ctx),
        );
    }
