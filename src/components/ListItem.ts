...
export class ListItem extends InteractiveLayoutElement
{
    ...
    set selected( yep:boolean ) {
        this._selected = yep;
        this.backgroundColor = yep? Theme.config.comboSelectedItemBgColor : undefined;
    ...
    override renderContents(ctx: CanvasRenderingContext2D, maxWidth: number, maxHeight: number): void {  
        this.writeText(ctx, this.label, this.fontSize*0.8*(this.isHeader?0.8:1), 0, maxHeight, this.fontColor)
    }
