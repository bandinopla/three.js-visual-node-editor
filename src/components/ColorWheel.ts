...
export class ColorWheel extends InteractiveLayoutElement {
    ...
    private createColorPalette( saturation:number ) {
        ...
        for (let y = 0; y < 200; y++) {
            ...
            const rgb = this._color.setHSL(hue / 360, saturation  , lightness / 100).toArray();
    ...
    protected override renderContents(ctx: CanvasRenderingContext2D, maxWidth: number, maxHeight: number): void {
        ...
        ctx.drawImage( this.colorPalette, 0, 0, 200,200, 0, 0, maxWidth, maxHeight);

         
        // mark...
        const w = 10;

        this.roundedRect(ctx, (this.hsl.h * maxWidth)-w/2, ((1-this.hsl.l) * maxHeight)-w/2, w, w, 15);
