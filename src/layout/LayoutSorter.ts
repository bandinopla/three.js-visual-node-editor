...
/**
 * This class is the one in charge to try to emulate the behaviour of the flexbox... It has a main axis and a secondary axis, it should
 * replciate the way flexbox sorts and aligns items... more work is needed...
 */
export class LayoutSorter {
    ...
    render(
        ...
        const items = childs.reduce<(LayoutElement | undefined)[]>((bag, child, i, arr) => {
            ...
            const lastChild = i + 1 == arr.length;
        ...
        items.forEach((elem, i, arr) => {
            ...
            const crossSize =
                this.align == "stretch"
                    ? maxCrossSize
                    : (elem ? this.getCrossSize(ctx, elem) : 0) || maxCrossSize;
            const crossFreeSize = maxCrossSize - crossSize;
            const crossOffset =
                crossFreeSize * (this.align == "end" ? 1 : this.align == "center" ? 0.5 : 0);
