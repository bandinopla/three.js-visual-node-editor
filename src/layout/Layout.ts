...
export class Layout extends LayoutElement {
    ...
    addChild(newChild: LayoutElement) {
        newChild.parent = this;
        this.childs.push(newChild);
    }

    removeChild(child: LayoutElement) {
    ...
    override height(ctx: CanvasRenderingContext2D) {
        ...
        if (this.config.direction == 'row') {
            ...
            //
            height = this.enabledChilds.reduce(
                (max, child) => Math.max(max, child.height(ctx)),
                0,
            );
        } else {
            //
            ...
            //
            height =
                this.enabledChilds.reduce(
                    (total, child) => total + child.height(ctx),
                    0,
                ) +
                this.config.gap * this.enabledChilds.length +
                this.config.gap;
        }
    ...
    override width(ctx: CanvasRenderingContext2D) {
        ...
        if (this.config.direction == 'column') {
            ...
            return this.enabledChilds.reduce(
                (max, child) => Math.max(max, child.width(ctx)),
                0,
            );
        }
    ...
    protected getChildsWidth(ctx: CanvasRenderingContext2D) {
        return (
            this.enabledChilds.reduce(
                (total, child) => total + child.width(ctx),
                0,
            ) +
            this.config.gap * this.childs.length +
            this.config.gap
        );
    }
    ...
    override render(
        ...
        if (this.config.offsetY) {
            ctx.translate(0, this.config.offsetY);
        }
    ...
    override renderContents(
        ...
        ctx.translate(0, this._scrollOffsetY);
    ...
    scrollContent(deltaY: number) {
        this._scrollOffsetY +=
            (deltaY > 0 ? -1 : 1) * 10 * this.config.scrollYSpeed;

        if (this.renderedHeight) {
            // minimum value
            const minY = -this._childsHeight + this.renderedHeight;

            if (this._scrollOffsetY < minY) {
                this._scrollOffsetY = minY;
            } else if (this._scrollOffsetY > 0) {
...
export class Row extends Layout {
    constructor(
        ...
        config?: Partial<Omit<LayoutConfig, 'direction'>>,
    ) {
        super(childs, {
            direction: 'row',
            ...config,
