...
export class LayoutElement<Ev extends {} = {}> extends CanvasElement<Ev> {
    ...
    protected set layout(customLayout: Layout | undefined) {
        ...
        if (customLayout) customLayout.parent = this;
    }
    ...
    set fontColor(color: FillStyle) {
        this._fontColor = color;
    }
    get fontColor() {
        return this._fontColor ?? this._parent?.fontColor ?? Theme.config.textColor;
    }

    set fontSize(fsize: number) {
        this._fontSize = fsize;
    }
    get fontSize() {
        return this._fontSize ?? this._parent?.fontSize ?? Theme.config.fontSize;
    }
    ...
    get rowHeight() {
        return this._rowHeight ?? this._parent?.rowHeight ?? Theme.config.nodeRowHeight;
    }

    set xMargin(newxMargin: number) {
        this._xMargin = newxMargin;
    }
    /**
     * Side space. Custom or inherited by a parent...
     */
    get xMargin() {
        return this._xMargin ?? this._parent?.xMargin ?? 0;
    }
