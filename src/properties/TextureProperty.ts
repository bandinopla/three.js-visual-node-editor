...
export class TextureProperty extends LayoutElement {
    ...
    constructor() {
        ...
        this.initial = new Layout(
            [
                new Button('+ File', () =>
                    this.onSelectFileFromDisk().then(
                        (file) => file && this.onFileSelected(file),
                    ),
                ),
        ...
        this._imageDisplayLayout = new Layout(
            [
                ...
                new Layout(
                    [this.filenameLabel, new Button('X', () => this.reset())],
                    {
                        justify: 'space-around',
                        align: 'center',
                    },
        ...
        this.overwritten = new Row([new TextLabel('Texture Data')]);
    ...
    protected askUserForUrl() {
        const url = prompt(
            "Paste image url to load from... Beware of CORS policy: If the other domain doesn't allow it the image will look black.",
            'http://',
        );
