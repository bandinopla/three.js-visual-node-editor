...
export class CanvasElement<T extends {} = {}> extends EventDispatcher<T> {
    /**
    ...
    protected drawPieChartCircle(
        ...
        colors.forEach((color, i) => {
            ...
            ctx.moveTo(x, y);
            ctx.fillStyle = color;
            ctx.arc(x, y, radius, i * sliceAngle, (i + 1) * sliceAngle);
            ctx.closePath();
            ctx.fill();
        });
    ...
    protected drawText(
        ...
        y: number,
        color: FillStyle,
        align: CanvasTextDrawingStyles['textAlign'] = 'left',
    ) {
        ctx.fillStyle = color;
        ctx.textAlign = align;
