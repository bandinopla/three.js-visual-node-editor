...
export class MeshStandardNode extends WinNode {
    ...
    constructor() {
        ...
        const metallic = new InputOrValue({
            label: 'Metallic',
            min: 0,
            max: 1,
            asBar: true,
        ...
        const roughness = new InputOrValue({
            label: 'Roughness',
            min: 0,
            max: 1,
            asBar: true,
        ...
        const normal = new BasicInputProperty(
            DataType.vec2,
            'Normal',
            () => '',
        );
    ...
    protected override writeNodeScript(script: Script) {
...
${metallicNode ? `material.metalnessNode = ${metallicNode};` : ''}
${roughnessNode ? `material.roughnessNode = ${roughnessNode};` : ''}
${normalNode ? `material.normalNode = ${normalNode};` : ''}
    ...
    override unserialize(data: Record<string, any>): void {
        ...
        if (data.baseColor)
            this.colorProperty.baseColor = new Color(data.baseColor);
